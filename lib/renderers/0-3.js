...
import { isValidSectionTagName, isValidMarkerType } from "../utils/tag-names";
...
function createElementFromMarkerType([tagName, attributes] = ["", []]) {
  ...
  for (let i = 0, l = attributes.length; i < l; i += 2) {
...
export default class Renderer {
  constructor(mobiledoc, state) {
    ...
    this.unknownCardHandler =
      unknownCardHandler || this._defaultUnknownCardHandler;
    this.unknownAtomHandler =
      unknownAtomHandler || this._defaultUnknownAtomHandler;
  ...
  get _defaultUnknownCardHandler() {
    return ({ env: { name } }) => {
      throw new Error(
        `Card "${name}" not found but no unknownCardHandler was registered`
      );
    };
  }
  ...
  get _defaultUnknownAtomHandler() {
    return ({ env: { name } }) => {
      throw new Error(
        `Atom "${name}" not found but no unknownAtomHandler was registered`
      );
    };
  }
  ...
  findCard(name) {
    ...
    if (name === HtmlCard.name) {
      return HtmlCard;
    }
    if (name === CodeCard.name) {
      return CodeCard;
    }
  ...
  renderCardSection([, index]) {
    let { card, payload } = this._findCardByIndex(index),
      cardWrapper = this._createCardElement(),
      cardArg = this._createCardArgument(card, payload),
      rendered = card.render(cardArg);
  ...
  _validateCardRender(rendered, cardName) {
    ...
    if (typeof rendered !== "string") {
      throw new Error(
        `Card "${cardName}" must render ${RENDER_TYPE}, but result was ${typeof rendered}"`
      );
    }
  }
  ...
  _validateAtomRender(rendered, atomName) {
    ...
    if (typeof rendered !== "string") {
      throw new Error(
        `Atom "${atomName}" must render ${RENDER_TYPE}, but result was ${typeof rendered}"`
      );
    }
  }
  ...
  _renderAtom(index) {
    let { atom, value, payload } = this._findAtomByIndex(index),
      atomArg = this._createAtomArgument(atom, value, payload),
      rendered = atom.render(atomArg);
  ...
  _renderMarkersOnElement(element, markers) {
    ...
    for (let i = 0, l = markers.length; i < l; i++) {
      let marker = markers[i],
        [type, openTypes, closeCount, value] = marker;
